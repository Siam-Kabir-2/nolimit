...
export default function AdminPanel({
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (username === DUMMY_USER && password === DUMMY_PASS) {
        ...
        setError("Invalid credentials. Try admin / nolimit2024");
      }
      setLoading(false);
    }, 800);
  };
  ...
  const quickLinks = [
    { label: "View All Models", href: "/inventory", icon: Car },
    { label: "Trade-In Requests", href: "/sell-my-car", icon: TrendingUp },
    { label: "Client Enquiries", href: "/finance", icon: Users },
  ];
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/8">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 rounded-lg bg-crimson/15 border border-crimson/25 flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-[9px] text-white/25 tracking-wider">NoLimit Control Panel</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        >
          {!authed ? (
            ...
            <div className="flex flex-col items-center justify-center h-full px-8 py-12">
              ...
              <h2 className="text-xl font-black text-white mb-1">Restricted Access</h2>
              ...
              <form onSubmit={handleLogin} className="w-full space-y-3">
                <input
                  ...
                  autoComplete="off"
                  className="w-full px-4 py-3.5 bg-white/4 border border-white/8 rounded-xl text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-crimson/40 transition-all"
                />
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3.5 pr-12 bg-white/4 border border-white/8 rounded-xl text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-crimson/40 transition-all"
                  />
                  <button
                    type="button"
                ...
                {error && (
                  <div className="flex items-center gap-2 p-3 rounded-xl bg-crimson/8 border border-crimson/20 text-[11px] text-crimson">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3.5 bg-crimson hover:bg-crimson-bright text-white text-[12px] font-bold uppercase tracking-[0.2em] rounded-xl transition-all duration-300 hover:shadow-[0_0_20px_oklch(60%_0.25_20/0.4)] disabled:opacity-60"
                >
            ...
            <div className="px-6 py-6 space-y-6">
              {/* Welcome */}
              <div className="flex items-center gap-3 p-4 rounded-xl bg-crimson/8 border border-crimson/15">
                <ShieldCheck className="w-4 h-4 text-crimson shrink-0" />
                <div>
                  <p className="text-[11px] font-bold text-white">Welcome, Administrator</p>
                  <p className="text-[10px] text-white/30">Full access granted</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  {[
                    { icon: LayoutDashboard, label: "Total Vehicles", value: totalVehicles, color: "text-white" },
                    { icon: Star, label: "Featured", value: featured, color: "text-amber-400" },
                    { icon: Zap, label: "New Arrivals", value: newArrivals, color: "text-crimson" },
                    { icon: TrendingUp, label: "Avg. Price", value: `$${(avgPrice / 1000).toFixed(0)}k`, color: "text-emerald-400" },
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {quickLinks.map(({ label, href, icon: Icon }) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Icon className="w-4 h-4 text-white/30 group-hover:text-crimson transition-colors" strokeWidth={1.5} />
              ...
              <div className="border-t border-white/5 pt-5 flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-3 rounded-xl bg-crimson/8 border border-crimson/15 hover:bg-crimson/15 transition-all text-[11px] font-medium text-crimson"
                >
