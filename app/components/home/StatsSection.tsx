...
const stats = [
  ...
  },
  {
    ...
    suffix: "€",
    label: "Acompte",
    sublabel: "Disponible dès maintenant",
    prefix: "",
  },
  {
    icon: Shield,
...
export default function StatsSection() {
  return (
    <section className="relative overflow-hidden bg-carbon py-16 sm:py-20 lg:py-28 pb-20 sm:pb-28">
      ...
      {/* Radial brand ambient */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_60%_at_50%_100%,oklch(65%_0.22_55/0.06),transparent)]" />

      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <AnimatedSection className="text-center mb-10 sm:mb-20">
          <div className="flex items-center justify-center gap-3 mb-3">
            <span className="block w-6 sm:w-8 h-px bg-brand" />
            <span className="text-[9px] sm:text-[10px] font-semibold tracking-[0.4em] uppercase text-brand">En Chiffres</span>
            <span className="block w-6 sm:w-8 h-px bg-brand" />
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight leading-none">
            La confiance des <span className="text-white/20">Passionnés</span>
          </h2>
          <p className="mt-3 text-xs sm:text-sm text-white/35 max-w-xs mx-auto leading-relaxed">
            Performance. Intégrité. Résultats.
          </p>
        </AnimatedSection>
            ...
            return (
              <AnimatedSection key={stat.label} delay={i * 0.12}>
                <div className="relative group p-5 sm:p-7 lg:p-9 rounded-2xl bg-surface border border-white/5
                             hover:border-brand/20 transition-all duration-500 text-center overflow-hidden">

                  {/* Glow behind card on hover */}
                  <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,oklch(65%_0.22_55/0.06),transparent)] opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

                  {/* Icon */}
                  <div className="relative inline-flex items-center justify-center w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-white/4 text-white/50 mb-4 sm:mb-6 group-hover:text-brand group-hover:bg-brand/10 transition-all duration-300">
                  ...
                  {/* Bottom accent line */}
                  <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-px bg-brand group-hover:w-1/2 transition-all duration-500" />
                </div>
              </AnimatedSection>
            );
          })}
